...
data = df.iloc[:,7:15].values

# 获取训练集
def get_train_data(batch_size=60, time_step=20, train_begin=0, train_end=4000):
    batch_index = []
...
# 获取测试集
def get_test_data(time_step=20, test_begin=4000):
...
def prediction(time_step=20):
    ...
    with tf.Session() as sess:
        ...
        test_y = np.array(test_y) * std[6] + mean[7]
        test_predict = np.array(test_predict) * std[6] + mean[7]
        print(test_predict)
        print(test_y)
